from pygame import mouse, event, display, QUIT, MOUSEBUTTONDOWN
...
class Player:
    def __init__(
        ...
        self._turn_state: Literal["Base","Swap","Blank"] = "Base"  # "Base" is placing tiles board, "Swap" is for using the swap tiles buttons, "Blank" is for using the blank tile buttons
        self._exit: bool = (
    ...
    def play_main(self) -> Literal["Continue", "Quit", None]:
        ...
        while self._is_turn:
            display.flip()
            mouse_coordinates: tuple[int,int] = mouse.get_pos()
            self._sidebar.recalculate_button_highlights(mouse_coordinates)
            if self._exit:
                self._screen_should_recompute
                return "Quit"
            ...
            for _event in event.get():
                if _event.type == QUIT:
                    ...
                    return_code: Literal["Continue", None]
                    if self._turn_state == "Base":
                        return_code = self.turn_execution_base(mouse_coordinates)
                        if return_code == "Continue":
                            return "Continue"
                    elif self._turn_state == "Swap":
                        return_code = self.turn_execution_swap(mouse_coordinates)
                    elif self._turn_state == "Blank":
                        return_code = self.turn_execution_blank(mouse_coordinates)
        display.flip()
        return
    
    def turn_execution_base(self, mouse_coordinates: tuple[int,int]) -> Literal["Continue", None]:
        display.flip()
        if (
            ...
            if self._tilerow.selected_tile_index != -1:
                # handle blank tile assignments
                if self._tilerow.selected_letter == " ": # a blank letter is currently selected
                    self._sidebar.blank_button_set.current_letter_button.text = "A"
                    self._sidebar.blank_button_set.current_letter_button.text_render()
                    self._sidebar.blank_button_set.current_letter_button.update()
                    self._sidebar.blank_button_set.hide_buttons()
                    self._turn_state = "Blank"
                    display.flip()
                    return
                ...
                assignment_worked: bool = (
                    self._game_board.set_letter_to_tile(
                        ...
                        self._tilerow.tile_list[self._tilerow.selected_tile_index] == " "
                    )
                )
                if assignment_worked:
                    ...
                    if (
                        ...
                        self._game_board.reset_tile(clicked_tile_coordinates)
                ...
                match clicked_button.text:
                    case "PLAY WORD":
                        ...
                        if attempt_success:
                            ...
                            Globals.scores[0] += attempt_information[1]
                            #self._sidebar.score_object.player_score += attempt_information[1]
                            self._sidebar.score_object.update()
                            self._is_turn = False
                            display.flip()
                            return "Continue"
                        else:
    ...
    def turn_execution_swap(self, mouse_coordinates: tuple[int,int]) -> None:
        display.flip()
    ...
    def turn_execution_blank(self, mouse_coordinates: tuple[int,int]) -> None:
        display.flip()         
        return_code = self._sidebar.blank_button_set.check_if_clicked(mouse_coordinates)

        if return_code:
            self.tilerow.selected_letter = return_code
            self._turn_state = "Base"
            self._sidebar.blank_button_set.hide_buttons()
        display.flip()
        

    def play(self) -> Literal["Continue","Quit", None]:
        print("player turn")
        self._is_turn: bool = True
        self._screen_should_recompute: bool = False
        while self._is_turn:
